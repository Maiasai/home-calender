...
export const GET = async (request:NextRequest,//ここのGETやPOSTでAppRouterのAPIはHTTPメソッドを判断してる
  {params} : { params : {id:string}} ) => {//
    //DBから一件だけ取得する

    const user = await requireUser();//認証チェック（関数コンポーネント）

    const recipe = await prisma.recipe.findUnique({ //DBから一意なキーで一件だけ取得（ここでの型はRecipe | null）
      where : { //URLで渡されたidのレシピを探して（ここで条件を渡してる）
        id : params.id//ここでidを指定
      },
      include : { //紐づく材料と手順もくっつけて
        recipeIngredients : {
          include : {
