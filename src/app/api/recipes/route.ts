...
export const GET = async (request:Request) =>{
  try{
    ...
    const recipeget = await prisma.recipe.findMany({//レシピテーブル全件を取得
      where: {
      ...
      //条件があるときだけ以下追加
        ...
        //お気に入り絞り込み
        ...(isFavorite === "true" && {
          userRecipeStatus:{//ログインユーザーのIDにisFavorite=trueが紐いているレシピだけとる
            some:{//「ログインユーザーの状態テーブルの中に1件でもtrueがあるか」
      ...
      select : { //includeは追加するものを指定する仕組みだが、selectは残すものを指定する仕組み
        id:true,
        title:true,
        category:true,
        thumbnailUrl:true,
        userRecipeStatus:{//userRecipeStatusはこのユーザーの分だけ返して（返す関連データを絞る）
          where:{
            userId:user.id,
          },
          select:{
...
//レシピ新規作成
interface CreatePostRequestBody { //bodyの形
  title : string
  memo? : string
  servings : number
...
export const POST  = async ( request:Request ) => {
  try{
    ...
    const { title , memo, servings , thumbnailImageUrl , ingredients , steps , category  } = body ;
  ...
  if (!body.ingredients || ingredients.length === 0){//材料が存在しない または　材料が空配列ならtrue
    return NextResponse.json (
      { message : " 材料は必須です " },
      { status : 400 }
    )
  ...
  const recipedata = await prisma.recipe.create ({ //テーブル操作。データベースに保存する処理
    data : {
      title,
      memo,
      servings,
  ...
  for ( let index = 0;  index < ingredients.length ; index++ ) { 
    ...
    console.log("ここ", ingre.amount)
    console.log("型", typeof ingre.amount)
    await prisma.recipeIngredient.create ({//③やってきた材料をDBに保存＞①からまた取り出してきて②→③と動いて保存される。（１つずつ）
      data : {
        quantityText : ingre.amount,
        sortOrder : index,
        

        recipe : {//recipeIngredientのrecipe(本来はrecipeIdをrecipe.idと接続したいが、connectが書けるのはリレーションのみだからrecipeと書く。)
          connect : { //　内部的には「recipeId = recipedata.id」となっている
            id: recipedata.id
          }//recipedata.id→前にcreateしたrecipeの戻り値からきてる。※　新しく作ったrecipeのid　→　recipedata = {id: "ckabc123",title: "カレー",...}
        },

        ingredient : {//ingredientテーブルに新しい材料を作って、 同時にその ingredientId を RecipeIngredient に入れる
          create : {
            name : ingre.name,
            normalizedName : ingre.name,
          }
        },
        unit : { //unitはすでにDBに存在しているためconnect
          connect : {
